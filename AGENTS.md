...
- Before any local Android device verification from source, run `pnpm build`, `pnpm build:android`, and `pnpm clean:daemon`. `build:android` refreshes and verifies both bundled Android helper artifacts for the current package version.
- Android verification must prove the helper path is active before the test run. Capture `snapshot -i --json` and require `androidSnapshot.backend` to be `android-helper` with `helperVersion` equal to `package.json`'s version. A stock UIAutomator fallback is not valid verification unless the fallback itself is the behavior under test.
